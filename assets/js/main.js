	...
	var BING_FALLBACK_GRADIENT = "linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)";

	function applyFallbackBackground(panel) {
		if (!panel) return;
		panel.style.backgroundImage = BING_FALLBACK_GRADIENT;
	...
	function getBingImages(imgUrls) {
		...
		// Preload image before applying; fall back on failure
		...
		preloader.onerror = function() {
			var remaining = imgUrls.filter(function(_, i) { return i !== index; });
			if (remaining.length > 0) {
				getBingImages(remaining);
			} else {
				applyFallbackBackground(panel);
			}
		};
		preloader.src = url;
	}
